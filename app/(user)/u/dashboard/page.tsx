...
export default async function UserDashboard() {
    ...
    if (!session || (session as any).user.role === 'admin') {
        // If not logged in or is admin (admins shouldn't be here?), redirect to login for now
        // Or if admin, redirect to admin dashboard?
        // Let's assume this is strictly for "user" role.
    ...
    const [user, combinedHistory] = await Promise.all([
        db.user.findUnique({
            ...
            include: {
                ...
                _count: {
                    select: { videoLikes: true, songLikes: true, playlists: true }
                }
            }
        }),
                    ...
                    <div className="flex-1 text-center md:text-left space-y-2">
                        ...
                        <div className="flex items-center justify-center md:justify-start gap-4 pt-2">
                            <div className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-sm font-medium">
                                <span className="text-blue-600 font-bold">{user._count.videoLikes + user._count.songLikes}</span> Likes
                            </div>
                            <div className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-sm font-medium">
                                <span className="text-purple-600 font-bold">{user._count.playlists}</span> Playlists
                            </div>
                        </div>
                    </div>
