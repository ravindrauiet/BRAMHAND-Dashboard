...
import { Play, Loader2, Save, ArrowLeft, Image as ImageIcon, Link as LinkIcon, Type, FileText, X, Upload, Trash2, List, Settings } from 'lucide-react';
...
export function SeriesEditor({ series, categories, creators }: SeriesEditorProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            {isEditing && (
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'details'
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'episodes'
                                ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm'
                                : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                            }`}
                    >
            ...
            {/* Episodes tab */}
            {activeTab === 'episodes' && isEditing && (
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-6">Manage Episodes</h2>
                    <EpisodesManager
                        seriesId={series.id}
