...
import { Play, Film, Loader2, Save, ArrowLeft, Image as ImageIcon, Link as LinkIcon, Type, FileText, Globe, Shield, Eye, Upload, X } from 'lucide-react';
...
export function VideoEditor({ video, categories, creators }: VideoEditorProps) {
    ...
    const uploadWithProgress = async (formDataToSend: FormData): Promise<any> => {
        return new Promise((resolve, reject) => {
            ...
            const url = isEditing ? `${apiUrl}/admin/videos/${video.id}` : `${apiUrl}/admin/videos`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formDataToSend.append('type', formData.type); // Enforce REEL
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-800 dark:text-white">{isEditing ? 'Edit Reel' : 'Upload Reel'}</h1>
                        <p className="text-slate-500 dark:text-slate-400 mt-1">Upload and manage your short video content.</p>
                    </div>
                    <Link href="/dashboard/reels" className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors text-slate-500">
                ...
                {isUploading && uploadProgress && (
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-lg">
                        <div className="flex justify-between items-center mb-2">
                            <span className="font-semibold text-slate-900 dark:text-white">Uploading...</span>
                            <span className="text-sm font-medium text-blue-600 dark:text-blue-400">{uploadProgress.percentage}%</span>
                        </div>
                        ...
                        <div className="flex justify-between text-xs text-slate-500">
                            <span>{formatBytes(uploadProgress.uploadedBytes)} of {formatBytes(uploadProgress.totalBytes)}</span>
                            <span>{formatTime(uploadProgress.estimatedTimeRemaining)} remaining ({formatBytes(uploadProgress.speed)}/s)</span>
                        </div>
                    </div>
                )}
                ...
                <div className="glass-card p-8 rounded-2xl space-y-6">
                    {/* Media Inputs (File Uploads) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 flex items-center gap-2">
                                <Upload className="w-4 h-4 text-pink-500" /> Video File
                            </label>
                            <input
                                type="file"
                                accept="video/*"
                            ...
                            {formData.videoUrl && (
                                <p className="text-xs text-emerald-600 mt-1 truncate">
                                    {videoFile ? `Selected: ${videoFile.name}` : 'Current URL present'}
                                </p>
                            )}
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 flex items-center gap-2">
                                <ImageIcon className="w-4 h-4 text-amber-500" /> Thumbnail
                            </label>
                            <input
                                type="file"
                                accept="image/*"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                {/* Correctly map creator user data if available, or fallback */}
                                {creators.map(c => (
                                    <option key={c.id} value={c.userId || c.id}>{c.user?.fullName || c.fullName || c.popularName || 'Unknown'}</option>
                                ))}
                            </select>
                        </div>
                        <div>
                    ...
                    <div className="flex flex-wrap gap-8 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                        <label className="flex items-center gap-3 cursor-pointer group">
                            ...
                            />
                            <span className="font-medium text-slate-700 dark:text-slate-300">Active Status</span>
                        </label>
                    ...
                    <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                        ...
                        >
                            ...
                            {isEditing ? 'Save Changes' : 'Publish Reel'}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-6">
                <div className="sticky top-6">
                    ...
                    {/* Preview Card */}
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-2xl max-w-[300px] mx-auto">
                        {/* Thumbnail */}
                        <div className="relative aspect-[9/16] bg-slate-100 dark:bg-slate-800 group cursor-pointer">
